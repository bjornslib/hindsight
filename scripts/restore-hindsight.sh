...
PG_BIN=$(for d in /home/hindsight/.pg0/installation/*/bin; do
    if [ -x "$d/psql" ]; then echo "$d"; break; fi
done)
if [ -z "$PG_BIN" ]; then
    echo "ERROR: Could not locate psql under /home/hindsight/.pg0/installation/*/bin" >&2
    exit 1
fi
